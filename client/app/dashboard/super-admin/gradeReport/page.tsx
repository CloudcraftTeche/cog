...
interface ExcelExportData {
  ...
  "Average Chapter Completion %": string;
  "Fully Completed Chapters": number;
  "Partially Completed Chapters": number;
  "Not Started Chapters": number;
  "Total Assignments": number;
  "Active Assignments": number;
  "Total Submissions": number;
  "Graded Submissions": number;
  "Pending Submissions": number;
  "Average Assignment Score": string;
...
export default function GradeCompletionReport() {
  ...
  useEffect(() => {
    fetchGradeCompletionReport();
  }, [currentPage, searchQuery]);
  const fetchGradeCompletionReport = async (): Promise<void> => {
    setLoading(true);
    try {
      const response = await api.get("/grades/completion-report", {
        params: {
          page: currentPage,
          limit: itemsPerPage,
          query: searchQuery,
      ...
      setTotalGrades(response.data.pagination.total);
      setTotalPages(response.data.pagination.totalPages);
    } catch (error) {
      console.error("Error fetching grade completion report:", error);
      toast.error("Failed to fetch grade completion report");
    } finally {
      setLoading(false);
    }
  };
  const exportToExcel = async (
    ...
    try {
      ...
      if (singleGrade) {
        ...
        gradesToExport = response.data.data;
      }
      if (gradesToExport.length === 0) {
        toast.error("No data to export");
        return;
      }
      const excelData: ExcelExportData[] = gradesToExport.map((grade) => ({
        Grade: grade.grade,
        "Academic Year": grade.academicYear || "N/A",
        "Teacher(s)":
          grade.teachers.length > 0
            ? grade.teachers.map((t) => t.name).join(", ")
          ...
          <>
                  ...
                  >
                    ...
                    <div className="p-6">
                      ...
                      <div className="mb-6 p-4 bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl">
                        ...
                        <p
                          className={`text-xs ${status.color} mt-1 font-medium`}
                        >
                          {status.text}
                        </p>
                      </div>
            ...
            <div className="flex items-center justify-between mt-8 bg-white p-4 rounded-xl shadow-lg">
              <div className="text-sm text-slate-600">
                Showing {grades.length} of {totalGrades} grades
              </div>
