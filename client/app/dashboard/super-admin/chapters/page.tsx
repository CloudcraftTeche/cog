...
import ChapterCard from "@/components/admin/chapters/ChapterCard";
interface Chapter {
  _id: string;
  title: string;
  description: string;
...
export default function SuperAdminChaptersPage() {
  ...
  const fetchChapters = async () => {
    try {
      ...
      const endpoint = "/chapters/chapters";
  ...
  const fetchGrades = async () => {
    try {
      ...
      console.log(
        "Grades with units:",
        sortedGrades.map((g: Grade) => ({
          grade: g.grade,
          _id: g._id,
          units: g.units?.map((u) => ({ _id: u._id, name: u.name })),
        }))
      );
  ...
  const handleDeleteChapter = async (chapterId: string) => {
    ...
    try {
      await api.delete(
        `/chapters/${chapter.gradeId._id}/chapters/${chapterId}`
      );
  ...
  const handleViewScores = (chapterId: string) => {
    router.push(`/dashboard/super-admin/chapters/scores/${chapterId}`);
  };
  const handleEdit = (chapterId: string) => {
    router.push(`/dashboard/super-admin/chapters/edit/${chapterId}`);
  };
  const gradeColors = [
