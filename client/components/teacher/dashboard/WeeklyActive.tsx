...
import { useWeeklyActiveStudents } from "@/hooks/teacher/useTeacherDashboard";
...
export const WeeklyActiveStudents: React.FC = () => {
  const { data: activeStudents = 0, isLoading, error } = useWeeklyActiveStudents();
  ...
  if (error) {
    return <ErrorDisplay message={error.message || "Failed to load weekly active students"} />;
  }

  const hasActiveStudents = activeStudents > 0;
      ...
      <div className="space-y-4">
        ...
        <div className="h-1 bg-white/20 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: hasActiveStudents ? "100%" : "0%" }}
          />
        </div>
      </div>
    </div>
