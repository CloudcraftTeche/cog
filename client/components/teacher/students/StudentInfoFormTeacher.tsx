...
import { GraduationCap, Upload, Lock } from "lucide-react"
import { FormErrors, StudentFormData } from "@/lib/studentValidation"
interface StudentInfoFormTeacherProps {
...
export const StudentInfoFormTeacher = ({
  formData,
  errors,
  gradeName,
  profilePicture,
    ...
    <Card className="border-0 shadow-2xl rounded-3xl overflow-hidden">
      ...
      <CardContent className="space-y-6 p-8 bg-gradient-to-br from-white to-gray-50">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            <Label htmlFor="gradeId" className="text-sm font-bold text-gray-700 flex items-center">
              Grade <Lock className="h-3 w-3 ml-1 text-gray-400" />
            </Label>
            <div className="relative">
              <Input
                ...
                value={gradeName}
                disabled
                className="rounded-xl border-2 border-gray-200 bg-gray-100 text-gray-600 cursor-not-allowed"
              />
              <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                <Lock className="h-4 w-4 text-gray-400" />
              </div>
            </div>
            <p className="text-xs text-gray-500">Students are added to your assigned grade</p>
          </div>
          <div className="space-y-3">
